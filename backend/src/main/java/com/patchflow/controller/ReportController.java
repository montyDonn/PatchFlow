        ...
        List<Task> tasks = allTasks.stream()
                ...
                .filter(t -> {
                    if ("CLIENT".equals(role))
                        return userId.equals(t.getAuthorId()) || userId.equals(t.getClientId());
                    if ("MANAGER".equals(role)) {
                        List<String> teamIds = userManagerRepository.findByManagerId(userId).stream()
                                .map(UserManager::getUserId).toList();
                ...
                .filter(t -> {
                    if ("weekly".equals(view))
                        return t.getCreatedAt().isAfter(Instant.now().minusSeconds(7 * 86400));
                    if ("monthly".equals(view))
                        return t.getCreatedAt().isAfter(Instant.now().minusSeconds(30 * 86400));
                ...
                .filter(t -> managerId == null
                        || t.getManagers().stream().anyMatch(m -> m.getUserId().equals(managerId)))
                .filter(t -> developerId == null
                        || t.getDevelopers().stream().anyMatch(d -> d.getUserId().equals(developerId)))
                .filter(t -> verifierId == null
                        || t.getVerifiers().stream().anyMatch(v -> v.getUserId().equals(verifierId)))
                .filter(t -> status == null || status.equals(t.getStatus()))
                .sorted(Comparator.comparing(Task::getCreatedAt).reversed())
                .collect(Collectors.toList());

        List<Map<String, Object>> normalized = tasks.stream().map(taskService::normalizeTask)
                .collect(Collectors.toList());
