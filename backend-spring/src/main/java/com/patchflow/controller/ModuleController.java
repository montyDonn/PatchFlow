...
public class ModuleController {
    ...
    public ResponseEntity<?> getHierarchy(HttpServletRequest req) {
        ...
        List<Map<String, Object>> result = modules.stream().map(m -> {
            ...
            List<Map<String, Object>> managers   = assignments.stream().filter(a -> List.of("MANAGER","ADMIN","SUPER_ADMIN").contains(((Map<?,?>)a.get("user")).get("role"))).collect(Collectors.toList());
