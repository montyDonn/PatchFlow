...
const serializeModule = (module: any) => ({
  ...
  users: module.users ? module.users.map((user: any) => ({
...
router.get('/hierarchy', authorize(["SUPER_ADMIN", "ADMIN"]), async (_req: Request, res: Response) => {
  try {
    const modules = await prisma.module.findMany({
      orderBy: { moduleName: 'asc' },
      include: { users: true },
...
router.get('/:moduleId', async (req: Request, res: Response): Promise<any> => {
  try {
...
// POST /api/modules
router.post('/', authorize(["SUPER_ADMIN", "ADMIN"]), async (req: AuthenticatedRequest, res: Response): Promise<any> => {
  try {
...
router.patch('/:moduleId', authorize(["SUPER_ADMIN", "ADMIN"]), async (req: AuthenticatedRequest, res: Response): Promise<any> => {
  try {
    const moduleId = String(req.params.moduleId);
    const { name, description, isActive } = req.body;
    const data: any = {};
    if (name !== undefined) data.moduleName = name;
...
router.delete('/:moduleId', authorize(["SUPER_ADMIN"]), async (req: AuthenticatedRequest, res: Response): Promise<any> => {
  try {
    const moduleId = String(req.params.moduleId);
